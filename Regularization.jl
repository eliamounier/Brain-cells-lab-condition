...
self_tuning_model_lasso = TunedModel(model = model_lasso, resampling = CV(nfolds = 10), tuning = Grid(), range = range(model_lasso, :lambda, scale = :log10, lower = 1e-8, upper = 1e-6), measure = MisclassificationRate())
...
# best lambda found by TunedModel is 1e-4

#lasso with PCA start 14:09
self_mach_lasso_PCA = machine(self_tuning_model_lasso, train_input_PCA, Y)
...
# best lambda found by TunedModel is 1e-6  
